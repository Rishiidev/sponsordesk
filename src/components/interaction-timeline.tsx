...
import type { Interaction, InteractionType, Deal, Contact } from "@/lib/db/local";
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mt-4 space-y-3 rounded-[8px] border border-[var(--color-line)] bg-[var(--color-paper-2)] p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-[var(--color-ink-2)]">Deal (optional)</label>
              <select
                name="dealId"
                defaultValue=""
                className="mt-1 block w-full rounded-[6px] border border-[var(--color-line)] bg-white px-3 py-2.5 text-[13px]"
      ...
      {interactions.length === 0 ? (
        <p className="mt-4 text-[13px] text-[var(--color-ink-3)]">No interactions logged yet. Capture the next email, call, or note here.</p>
      ) : (
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-0.5 text-[11px] text-[var(--color-ink-3)]">
                    ...
                    {deal && (
                      <>
                        {" · "}
                        <a href={`/deals/${deal.id}`} className="text-[var(--color-accent)] hover:underline">
                          {deal.title}
                        </a>
                      </>
                    )}
